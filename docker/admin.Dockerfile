# SpreddPay operations portal.
#
# Same single-stage rationale as docker/api.Dockerfile.
# Railway sets RAILWAY_DOCKERFILE_PATH=docker/admin.Dockerfile on the service.
...
# The admin app imports @spreddpay/contracts, which does not need Prisma — but the
...
RUN pnpm --filter @spreddpay/admin build

ENV NODE_ENV=production

# `next start` with no --port reads $PORT, which Railway injects.
CMD ["pnpm", "--filter", "@spreddpay/admin", "start"]
