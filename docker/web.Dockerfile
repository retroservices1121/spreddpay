# SpreddPay trader portal (white-label).
#
# Same single-stage rationale as docker/api.Dockerfile.
# Railway sets RAILWAY_DOCKERFILE_PATH=docker/web.Dockerfile on the service.
...
# The web app imports @spreddpay/contracts, which does not need Prisma — but the
...
RUN pnpm --filter @spreddpay/web build

ENV NODE_ENV=production

# `next start` with no --port reads $PORT, which Railway injects.
CMD ["pnpm", "--filter", "@spreddpay/web", "start"]
